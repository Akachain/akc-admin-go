...
func (c *Config) GetContextByName(contextName string) (*Context, error) {
...
func (c *Config) GetNetworkByContext(contextName string) (*Network, error) {
	context, err := c.GetContextByName(contextName)
	if err != nil {
		return nil, err
	}

	if len(context.Network) == 0 {
		return nil, errors.New("no network is set for this context")
	}

	network, ok := c.Networks[context.Network]
	if !ok {
		return nil, errors.New("network of context does not exist")
	}

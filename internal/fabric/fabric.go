...
func NewFactory(config *environment.Config, contextName string) (Factory, error) {
	context, err := config.GetContextByName(contextName)
	if err != nil {
		return nil, err
	}

	network, err := config.GetNetworkByContext(contextName)
	if err != nil {
		return nil, err
	}
...
func NewFactoryDefault(config *environment.Config) (Factory, error) {
